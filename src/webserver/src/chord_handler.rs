...
pub fn init_chord(me: NodeAddr, mut all_nodes: Vec<NodeAddr>) -> ChordHandler {
    ...
    let me_index = nodes.iter().position(|n| n.id == me_id).expect("Failed to find own node"); // find our index in the list
    ...
    for i in 0..M {
        ...
        let finger_node = match nodes.binary_search_by(|n| {
            if n.id < start { Ordering::Less } 
            else if n.id > start { Ordering::Greater } 
            else { Ordering::Equal }
        }) {
...
impl ChordHandler {
    ...
    pub fn responsible_for(&self, key: &str) -> bool {
        in_interval_open_closed(hash_key(key), self.network.predecessor.id, self.network.me.id)
    }
...
pub async fn forward_get(chord: &ChordHandler, key: &str, hop_count: u32) -> actix_web::Result<HttpResponse> {
    ...
    let response = chord.client.get(url)
    ...
    match response {
        Ok(r) => {
            ...
            let body = r.bytes().await.unwrap_or_else(|_| Bytes::from_static(b"Error reading body"));
...
pub async fn forward_put(chord: &ChordHandler, key: &str, value: Bytes, hop_count: u32) -> actix_web::Result<HttpResponse> {
    if hop_count >= HOP_LIMIT {
        return Ok(HttpResponse::BadGateway().body("Chord hop limit exceeded")); // Prevent infinite loops
    }
    ...
    let response = chord.client.put(url)
        .header("X-Chord-Hop-Count", (hop_count + 1).to_string())
        .timeout(std::time::Duration::from_millis(1000))
        .body(value.clone())
        .send()
